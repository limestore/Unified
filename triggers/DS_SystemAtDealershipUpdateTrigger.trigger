trigger DS_SystemAtDealershipUpdateTrigger on System_at_Dealership__c (before update) {
      ...
      if (Trigger.isBefore){
            ...
            for(integer i=0; i<trigger.new.size();i++){
                if (trigger.new[i].System_Type__c == trigger.old[i].System_Type__c || trigger.new[i].System_Type__c == 'Previous ' + trigger.old[i].System_Type__c || trigger.old[i].System_Type__c == 'Previous ' + trigger.new[i].System_Type__c) {
                    // Do Nothing here
                }else{
                    //cancel update
                ...
                if(trigger.old[i].System_Type__c == 'Previous ' + trigger.new[i].System_Type__c){
                    Trigger.new[i].addError('The System Type can not be changed from Previous to Current. Please add a new record with the desired Current System Type.');  
                }


                  if (trigger.new[i].System_Type__c == trigger.old[i].System_Type__c || trigger.new[i].System_Type__c == 'Previous ' + trigger.old[i].System_Type__c || trigger.old[i].System_Type__c == 'Previous ' + trigger.new[i].System_Type__c) {
                        //should not allow changing type except to/from(?) previous                   
                        if (trigger.new[i].Vendor__c != trigger.old[i].Vendor__c || trigger.new[i].System_Type__c != trigger.old[i].System_Type__c || trigger.new[i].Install_Date__c != trigger.old[i].Install_Date__c) {
                              Systems__c[] curSys1 = [select s.Id, s.SLXSystem_ID__c, s.SLXVendor_ID__c, s.Type__c from Systems__c s where s.Id = :trigger.new[i].Vendor__c];                              
                              ...
                              if (trigger.new[i].CompositeExtID__c == null || trigger.new[i].CompositeExtID__c != trigger.new[i].Account__c + '' + strIdate +  curSys1[0].SLXVendor_ID__c + '' + strDMS + '' + trigger.new[i].System_Type__c) {
                                    trigger.new[i].CompositeExtID__c = trigger.new[i].Account__c + '' + strIdate +  curSys1[0].SLXVendor_ID__c + '' + strDMS + '' + trigger.new[i].System_Type__c;
                              }
                              if (trigger.new[i].SLX_Vendor_ID__c == null || trigger.new[i].SLX_Vendor_ID__c != curSys1[0].SLXVendor_ID__c) {
